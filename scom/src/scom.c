...
// Rev. 25.9.2020, JKJ: Fixed buffer size for otp register decode string - bumped version to 0.63 
// Rev. 27.10.2020, JKJ: Block write address need to ignore bit 7 of page - bumped version to 0.64 
                ...
                { printf("\n%s>",cmdarg[1]);
                  ...
                  switch (key)
                    {   case '+' :  if ( trimval <  ((1<<reg.size)-1) )
                        {  trimval = trimval + 1;
                          sprintf(cmd,"scom w %d 0x%02x %s",reg.adr, (regval & umask) | (trimval<<reg.pos) , cli_arg );
                          system(cmd);
                        }
                        break;
                    case '-' :  if ( trimval > 0 )
  ...
  if (otp_switch == 1)                                                 // process write buffer for otp write
    {
      ...
      int otp_entries[2];
      unsigned char symbol_view[2024]; 
	  ...
	  for(int i = 0; i <= otp_entries[0]/16;i++)
	  { printf("0x%04x : ", i*16+0x8000);
	    for (j = 0; j < 16; j++)
		{ printf("%02x ",otp_mem[i*16+j]);
		} 
		printf("\n"); 
      } 	   
      ...
      { printf("Read error double otp read out do not match!\n");
        exit(1);
      }
        ...
        { printf("0x%04x ",i+0x8000);
          if ((otp_mem[i] & 0x80) == 0x00)                                                // Single write
            ...
            } else if  ((otp_mem[i] & 0xe0) == 0xc0) 
            {
              ...
              for (j=0;j<len;j++)
                { lookup_symbol(symbol_view, (otp_mem_page & 0xff00)+otp_mem[i+1]+j , otp_mem[i+j+2] );
                  //strcpy(symbol_view,"\n"); 
				  printf("0x%04x : 0x%02x        cont         0x%04x 0x%02x  %s", (i+j+2+0x8000), otp_mem[i+j+2], 
                         (otp_mem_page & 0xff00) + otp_mem[i+1] + j ,
                         otp_mem[i+j+2],
                         symbol_view ); 
                }
              i = i+j+1;
